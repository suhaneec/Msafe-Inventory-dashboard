...
.sec-sub { font-size: 0.82rem; color: #6B6458 !important; margin-bottom: 0.9rem; }
.chart-heading {
    font-family: 'Fraunces', serif;
    font-size: 1.05rem;
    font-weight: 700;
...
.chart-caption { font-size: 0.8rem; font-weight: 600; color: #6B6458 !important; margin-bottom: 0.4rem; }
...
section[data-testid="stSidebar"] input { background-color: #243648 !important; color: #fff !important; }
section[data-testid="stSidebar"] [data-baseweb="tag"] {
    background-color: #B5562B !important;
    border-color: #B5562B !important;
}
section[data-testid="stSidebar"] [data-baseweb="tag"] * { color: #fff !important; fill: #fff !important; }
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] {
    background-color: #243648 !important;
    border-color: #354A60 !important;
}
section[data-testid="stSidebar"] [data-testid="stFileUploaderFile"] { background-color: #243648 !important; }
...
/* ── Floating fallback button — always visible, fixed position ─
   This renders a real clickable button at the left edge of the
   screen that opens the sidebar by dispatching a click on the
   native Streamlit toggle. Works regardless of what Streamlit
   names its internal elements across versions.               */
#msafe-sidebar-btn {
    position: fixed;
    top: 50%;
    left: 0;
...
#msafe-sidebar-btn:hover { background: #B5562B; }
</style>

<button id="msafe-sidebar-btn" onclick="openSidebar()" title="Open filters">☰ Filters</button>
...
function openSidebar() {
    ...
    // Last resort: find any button containing a chevron SVG near left edge
    var btns = document.querySelectorAll('button');
    for (var j = 0; j < btns.length; j++) {
        var b = btns[j];
        var rect = b.getBoundingClientRect();
        if (rect.left < 60 && b.querySelector('svg')) {
            b.click();
            return;
        }
    }
...
// Also hide our custom button once the real sidebar is open
(function watchSidebar() {
    var btn = document.getElementById('msafe-sidebar-btn');
    if (!btn) return;
    var observer = new MutationObserver(function() {
        var sidebar = document.querySelector('section[data-testid="stSidebar"]');
        if (sidebar) {
            var style = window.getComputedStyle(sidebar);
            // Hide our btn when sidebar is visible
            btn.style.display = (style.display === 'none' || sidebar.offsetWidth < 10) ? 'block' : 'none';
        }
    });
...
def parse_location_sheet(rows, tab_name):
    ...
    def _age_bucket(r):
        if r["Qty_180_plus"] > 0: return "180+ days"
        if r["Qty_120_180"] > 0:  return "120–180 days"
        if r["Qty_60_120"] > 0:   return "60–120 days"
        if r["Qty_0_60"] > 0:     return "0–60 days"
        return "180+ days"
...
def horizontal_bar_chart(df, label_col, value_col, accent_color, value_format="₹{:,.0f}"):
    ...
    text = bars.mark_text(align="left", dx=8, fontSize=14, fontWeight="bold", color="#1A2B3C"
    ).encode(text="_value_label:N")
    chart = (bars + text).properties(
        height=max(34 * len(chart_df), 140), width="container",
        padding={"top": 6, "right": 12, "bottom": 4, "left": 4}
    ).configure_view(strokeWidth=0, fill="#FAF8F4", clip=False
    ).configure(background="#FAF8F4"
    ).configure_axis(labelColor="#1A2B3C", titleColor="#1A2B3C"
    ).configure_text(color="#1A2B3C")
...
with st.sidebar:
    ...
    uploaded = st.file_uploader(
        ...
        help="ERP export of stock not moved in 60+ days."
    )
    st.markdown("---")
...
<div class="explain-box">
  ...
  for at least <b>120 days</b> — it isn't earning rental income. Anything idle
  <b>{threshold_days}+ days</b> is counted as <b>"Dead Stock"</b> — money tied up with no return.
</div>
""", unsafe_allow_html=True)
...
if len(dead_v) > 0:
    worst_product = (
        ...
        .reset_index().sort_values("Value", ascending=False).iloc[0]
    )
...
def render_age_band_body(band_df, band_label, band_key, accent_color):
    ...
    <div class="kpi-row" style="margin-bottom:1.1rem;">
      <div class="kpi"><div class="lbl">Items in this band</div>
        <div class="val">{len(band_df):,}</div><div class="sub"><b>{band_qty:,} units</b></div></div>
      <div class="kpi"><div class="lbl">Value locked up</div>
        <div class="val">₹{band_value/100000:.1f}L</div><div class="sub">in this age band alone</div></div>
    ...
    with yc1:
        ...
        c1 = yard_units.reset_index(); c1.columns = ["Yard", "Units"]
        horizontal_bar_chart(c1, "Yard", "Units", accent_color, value_format="{:,.0f} units")
    with yc2:
        st.markdown('<div class="chart-caption">By value (₹) — highest first</div>', unsafe_allow_html=True)
        c2 = yard_value.reset_index(); c2.columns = ["Yard", "Value"]
        horizontal_bar_chart(c2, "Yard", "Value", accent_color, value_format="₹{:,.0f}")

    yard_table = pd.DataFrame({"Yard": yard_value.index, "Value ₹": yard_value.values}).merge(
        pd.DataFrame({"Yard": yard_units.index, "Units": yard_units.values}), on="Yard"
    ...
    prod_grp = band_df.groupby(["Item Code", "Item Name"]).agg(
        Units=("Idle Qty", "sum"), Value=("Idle Value", "sum"), Yards=("Location", "nunique"),
    ).reset_index()
    pc1, pc2 = st.columns(2)
    with pc1:
        st.markdown('<div class="chart-caption">Top 10 by units</div>', unsafe_allow_html=True)
        horizontal_bar_chart(prod_grp.sort_values("Units", ascending=False).head(10),
                             "Item Name", "Units", accent_color, value_format="{:,.0f} units")
    with pc2:
        st.markdown('<div class="chart-caption">Top 10 by value (₹)</div>', unsafe_allow_html=True)
        horizontal_bar_chart(prod_grp.sort_values("Value", ascending=False).head(10),
                             "Item Name", "Value", accent_color, value_format="₹{:,.0f}")
    ...
    st.dataframe(prod_table[["Rank","Item Code","Item Name","Yards","Units","Value ₹"]].rename(
        columns={"Yards": "# Yards Affected"}), width="stretch", hide_index=True)

    buf = write_xlsx_stdlib({
        f"{band_key} - By Yard": df_to_rows(yard_table[["Rank","Yard","Units","Value ₹"]]),
        f"{band_key} - By Product": df_to_rows(prod_table[["Rank","Item Code","Item Name","Yards","Units","Value ₹"]]),
    })
    st.download_button(f"⬇️  Download {band_label} report (.xlsx)", data=buf,
...
def render_analysis_tab(master_v, dead_v, band_120_180, band_180_plus, threshold_days):
    ...
    st.markdown('<div class="sec-sub">Auto-generated from the current upload</div>', unsafe_allow_html=True)
    if len(master_v) == 0:
    ...
    if len(yard_dead_value) > 0:
        ...
        n_yd = (yard_dead_value > 0).sum()
        _insight_card("🏭", f"{top_yard} is the biggest concentration of dead stock",
            f"Accounts for <b>₹{top_yard_value/100000:.1f}L</b> — <b>{top_yard_pct:.0f}%</b> of all "
            f"Dead Stock across {n_yd} yards. Top 3 yards = <b>{top3_pct:.0f}%</b> of total.")
    ...
    if len(dead_v) > 0:
        ...
        tp = prod_dead.iloc[0]
        t5 = prod_dead.head(5)["Value"].sum() / dead_value * 100 if dead_value else 0
        _insight_card("📦", f"{tp['Item Name']} is the worst product by value",
            f"<b>₹{tp['Value']:,.0f}</b> locked up across <b>{int(tp['Yards'])} yard(s)</b>. "
            f"Top 5 products = <b>{t5:.0f}%</b> of all Dead Stock.")

    if len(dead_v) > 0:
        cross = dead_v.groupby(["Item Code","Item Name"])["Location"].nunique().reset_index(name="Yards")
        multi = cross[cross["Yards"] >= 3]
        if len(multi) > 0:
            _insight_card("🔁", f"{len(multi)} products are dead in 3+ yards simultaneously",
                "A company-wide call needed — not a yard-level fix. See <b>Repeat Offenders</b> tab.")

    if len(band_120_180) > 0:
        tv = band_120_180["Idle Value"].sum()
        ti = len(band_120_180)
        _insight_card("⏳", f"₹{tv/100000:.1f}L is one step from becoming Dead Stock",
            f"<b>{ti} items</b> in 120–180 day band. Without action in 1–2 months, "
            f"this crosses the {threshold_days}-day line.")

    dpct = dead_value / total_value * 100 if total_value else 0
    _insight_card("💰", f"Dead Stock is {dpct:.0f}% of all tracked idle value",
        f"<b>₹{total_value/100000:.1f}L</b> tied up across <b>{n_yards} yards</b>. "
        f"<b>₹{dead_value/100000:.1f}L</b> has crossed the {threshold_days}-day line.")

    st.markdown('<div class="chart-heading">Dead Stock value by yard</div>', unsafe_allow_html=True)
    if len(yard_dead_value) > 0:
        cdf = yard_dead_value.reset_index(); cdf.columns = ["Yard", "Value"]
        horizontal_bar_chart(cdf, "Yard", "Value", "#B5562B", value_format="₹{:,.0f}")
    else:
    ...
    if len(dead_v) > 0:
        tpc = dead_v.groupby(["Item Code","Item Name"])["Idle Value"].sum().sort_values(ascending=False).head(10).reset_index()
        horizontal_bar_chart(tpc, "Item Name", "Idle Value", "#B5562B", value_format="₹{:,.0f}")
...
with tab_120:
    ...
    st.markdown('<div class="sec-sub">About to cross into Dead Stock — window to act is closing</div>', unsafe_allow_html=True)
...
with tab_byyard:
    ...
    st.markdown(f'<div class="sec-sub">Worst yards first — {threshold_days}-day cutoff</div>', unsafe_allow_html=True)
    if len(dead_v) == 0:
        st.success(f"No yard currently has stock idle {threshold_days}+ days.")
    else:
        loc_dead = dead_v.groupby("Location").agg(
            Items=("Item Code","count"), Qty=("Idle Qty","sum"), Value=("Idle Value","sum")
        ).reset_index()
        ...
        st.dataframe(loc_dead[["RAG","Location","Items","Qty","Value ₹","% of yard's idle items"]].rename(
            columns={"Location":"Yard","Qty":"Units"}), width="stretch", hide_index=True)
        ...
        for loc in loc_dead["Location"]:
            ...
            with st.expander(f"📍 {loc}  —  {len(loc_df)} dead items  ·  ₹{loc_df['Idle Value'].sum():,.0f}", expanded=False):
                ...
                st.dataframe(show[["RAG","Item Code","Item Name","Age Bucket","Qty","Idle Value"]],
                             width="stretch", hide_index=True)

with tab_repeat:
    st.markdown('<div class="sec-title">Items dead in many yards at once</div>', unsafe_allow_html=True)
    st.markdown('<div class="sec-sub">Company-wide issue — not just one yard\'s problem</div>', unsafe_allow_html=True)
    if len(dead_v) == 0:
        ...
        cross = dead_v.groupby(["Item Code","Item Name"]).agg(
            ...
            Qty=("Idle Qty","sum"), Value=("Idle Value","sum"),
        ).reset_index().sort_values(["Yards","Value"], ascending=[False,False])
        multi = cross[cross["Yards"] >= 3]
        if len(multi):
            st.markdown(f"""<div class="alert-box"><b>{len(multi)} items</b> are dead stock in
              <b>3 or more yards at the same time.</b> Worth a central review.</div>""",
              unsafe_allow_html=True)
        cross["RAG"] = cross["Yards"].apply(lambda x: "🔴" if x >= 5 else ("🟠" if x >= 3 else "🟡"))
        cross["Value ₹"] = cross["Value"].apply(lambda x: f"₹{x:,.0f}")
        st.dataframe(cross[["RAG","Item Code","Item Name","Yards","Yard_List","Qty","Value ₹"]].rename(
            columns={"Yards":"# Yards","Yard_List":"Found In"}), width="stretch", hide_index=True)
        buf2 = write_xlsx_stdlib({"Repeat Offenders": df_to_rows(cross[["Item Code","Item Name","Yards","Yard_List","Qty","Value"]])})
        st.download_button("⬇️  Download this list (.xlsx)", data=buf2,
